...
## 33. Recette HTTP réelle et préparation mobile
...
Ce contrôle complète la recette GoTrue/Storage annoncée au §31, pour deux administrateurs et le périmètre documenté. Une visite mobile fictive a été préparée ; le relevé serveur la montre ensuite en cours avec une première réponse. L’utilisateur dispose d’un iPhone ou d’un iPad, mais les gestes physiques, la coupure réseau, la reprise et les observations restent à consigner avant de déclarer la recette terrain terminée. Aucun changement applicatif ni migration pour ce lot documentaire ; publier les spécifications après CI et actualiser le dépôt VPS, sans reconstruire une image identique.

**Suivi Safari :** l’utilisateur déclare avoir effectué la manipulation dans Safari. Deux visites du chantier de recette sont désormais clôturées ; la seconde contient une réponse et une photo JPEG dont l’existence est vérifiée (HTTP 200, 795 705 octets). Son PDF a été généré sans email. Aucune remarque n’est enregistrée. La coupure effective (mode avion et Wi-Fi désactivé), la saisie pendant cette coupure et la conservation après rechargement physique restent à confirmer ; ne pas déclarer le scénario hors ligne entièrement validé sur la seule présence du fichier serveur. Détails et distinction entre déclaration utilisateur et preuve serveur dans le procès-verbal.
