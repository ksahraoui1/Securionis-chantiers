...
export function EcartPieces({ecartId,auteurId,entrepriseId,revision,pieces,setPieces,disabled,peutAjouter,onActivite}:{ecartId:string;auteurId:string;entrepriseId:string;revision:number;pieces:PieceBrouillon[];setPieces:Dispatch<SetStateAction<PieceBrouillon[]>>;disabled:boolean;peutAjouter:boolean;onActivite:(active:boolean)=>void}) {
 ...
 async function envoyer(p:PieceBrouillon) {
  ...
  try {
   const response=await fetch(`/api/ecarts/${ecartId}/pieces`,{method:"POST",headers:{"Content-Type":"application/octet-stream","X-Piece-Id":p.id,"X-Auteur-Id":auteurId,"X-Entreprise-Id":entrepriseId,"X-Revision":String(p.revision),"X-Nom-Fichier":encodeURIComponent(p.file.name)},body:p.file});
   let data;
  ...
  } catch(e) {setPieces(old=>old.map(x=>x.id===p.id?{...x,erreur:(e as Error).message}:x));}
 ...
 return <fieldset disabled={disabled} className="space-y-3 rounded-lg border border-gray-300 p-3">
  ...
  <ul className="space-y-3">{pieces.map(p=><li key={p.id} className="text-sm space-y-1">
   ...
   {p.revision!==revision?<p className="text-amber-800">Le suivi a changé. Retirez cette sélection puis choisissez à nouveau le fichier.</p>:p.piece?<p className="text-green-700">Pièce prête pour la soumission.</p>:p.erreur?<p role="status" className="text-amber-800">{p.erreur}</p>:<p className="text-gray-500">À envoyer.</p>}
