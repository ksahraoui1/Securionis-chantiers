...
interface Etat { ecart: {id:string;statut:string}; suivi:SuiviEcart|null; historique:EvenementEcart[]; peutModifier:boolean; auteurId:string;entrepriseId:string }
export function EcartCycle({ecartId,auteurId}:{ecartId:string;auteurId:string}) {
 ...
 async function envoyer(action:ActionCycle, retry=false) {
  ...
  try {
   ...
   setPending(null);if(demande.action!=="planifier")setCommentaire("");setMessage("Action enregistrée dans l’historique.");
 ...
 const verrouille=busy||!!pending||conflit;
  ...
  {etat && <>
   ...
   {etat.peutModifier && statut!=="corrige" && <>
    ...
    {(statut==="en_cours_correction" && !!etat.suivi || statut==="a_verifier") && <fieldset disabled={verrouille} className="space-y-3">
     ...
     <div className="flex flex-wrap gap-2">{statut==="a_verifier"?<><button disabled={verrouille} className={bouton} onClick={()=>envoyer("valider")}>Valider la correction</button><button disabled={verrouille} className={bouton} onClick={()=>envoyer("reprendre")}>Demander une reprise</button></>:<button disabled={verrouille || responsable!==etat.suivi?.responsable || echeance!==etat.suivi?.echeance} className={bouton} onClick={()=>envoyer("soumettre")}>Soumettre à vérification</button>}</div>
