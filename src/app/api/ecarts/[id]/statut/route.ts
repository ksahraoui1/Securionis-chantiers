...
export async function GET(request: NextRequest, {params}:{params:Promise<{id:string}>}) {
  try {
    ...
    let historiqueQuery=client.from("ecart_evenements").select("id,revision,auteur_nom,action,statut_avant,statut_apres,responsable,echeance,commentaire,created_at,ecart_pieces(id,nom,mime,taille,sha256,revision_preparation)").eq("ecart_id",id).order("revision",{ascending:false}).limit(50);
    ...
    let piecesActuelles: PieceEcart[] = [];
    if(suivi?.soumission_id) {
      const {data:pieces,error:piecesError}=await client.from("ecart_pieces").select("id,nom,mime,taille,sha256,revision_preparation").eq("evenement_id",suivi.soumission_id).eq("ecart_id",id);
      if(piecesError) return json({error:"Preuves indisponibles."},503);
      piecesActuelles=pieces??[];
    }
    ...
    return json({ecart,suivi,historique,piecesActuelles,peutModifier,auteurId:profil.id,entrepriseId:profil.entreprise_id});
...
export async function PATCH(request: NextRequest, {params}:{params:Promise<{id:string}>}) {
  try {
    ...
    const {data,error}=await client.rpc("avancer_cycle_ecart_v2",{p_ecart_id:id,p_operation_id:demande.operationId,p_revision:demande.revision,p_action:demande.action,p_responsable:demande.responsable,p_echeance:demande.echeance,p_commentaire:demande.commentaire,p_pieces:demande.pieces??[]});
    if(error) {
