...
Le troisième lot isole IndexedDB et les préférences de visite avec `OfflineScope` (compte + entreprise). Tout appel au stockage hors ligne doit porter ce périmètre capturé ; les envois passent par `createOfflineClient`, jamais par un client de session mutable. Conserver les réponses/photos jusqu’à leur accusé exact de révision. Ne pas réattribuer ni purger la base historique sans propriétaire. Le service worker v10 ne cache que les ressources publiques : ne pas réintroduire de cache HTML/RSC ou média privé. Les tests de régression sont aussi dans `tests/offline-security.test.cjs`. Les conflits serveur des réponses et S09 restent à traiter ; la clôture atomique est couverte par le lot suivant.

Le quatrième lot (migration 059) impose `preparer_cloture_visite` puis `cloturer_visite`. Ne jamais réintroduire une création de NC suivie d’un UPDATE de statut côté client. La procédure compare les constats sous verrou, enregistre leur clôture atomiquement et reconnaît le rejeu de la même demande. Les triggers figent les réponses et les données de visite terminée ; seul le suivi des corrections, des rapports et des emails reste modifiable. Conserver l’UUID et le contenu de la demande après une issue réseau incertaine. Tests SQL : `tests/security-db-cloture.sql`, puis `tests/security-db-concurrency.sh`, uniquement sur une base jetable. Les photos sources, le référentiel complet et les avenants restent des limites S06.

Le cinquième lot (migration 060) remplace l’upsert des réponses par `synchroniser_reponse`, avec comparaison de `sync_revision` sous verrou, horodatage serveur et rejeu du dernier envoi exact. Les écritures directes des rôles API sont refusées ; les anciens onglets doivent être rechargés. Un conflit `40001` conserve toute la file locale. La révision est encore lue au début de l’envoi : la version de départ d’une ancienne saisie hors ligne et l’interface de résolution restent à implémenter (S07 partiel). Tests : suites de clôture, puis `tests/security-db-sync.sql` et `tests/security-db-sync-concurrency.sh` sur la même base jetable. Un retour à une ancienne image ne rétablit pas les écritures directes ; conserver la migration et corriger en avant.

Le sixième lot (migration 061) réserve les envois à `synchroniser_reponse_v2` et ferme l’ancienne entrée aux clients. `base_revision` vient des valeurs affichées avant l’édition : `null` signifie absence observée, `undefined` une ancienne saisie non vérifiable, jamais une permission de prendre la révision distante. Les accusés avancent uniquement leurs descendants locaux ; conserver les reçus jusqu’au départ du formulaire. Deux formulaires concurrents conservent leurs brouillons séparément. Les copies de récupération bloquent aussi la clôture et sont exportables localement avec les photos. Ne pas purger ces copies ni réinitialiser leur origine. L’interface de résolution/import reste à construire.

Le propriétaire a aussi confirmé la suppression des 91 réponses `pas_necessaire` historiques le 14 septembre 2026. Le script ponctuel `scripts/maintenance/20260914-supprimer-reponses-non-necessaires.sql` vérifie leur empreinte et sauvegarde les lignes dans `maintenance_privee.reponses_non_necessaires_20260914` (aucun accès API), puis supprime sous verrou exclusif et rétablit le trigger avant commit. Ce script ne fait pas partie des migrations automatiques et refuse un rejeu. Ne pas généraliser cette exception de maintenance aux clients. Les 95 visites, 83 écarts, les fichiers et les PDF archivés ont été conservés ; 122 réponses restent actives.

Le septième lot (062) impose une frontière d’entreprise restrictive aux tables et au stockage. Ajouter cette frontière et le MFA à toute nouvelle table ; vérifier tous les parents en base et le périmètre des procédures `SECURITY DEFINER`. Ne jamais utiliser le seul rôle administrateur comme autorisation sur un UUID. Les transferts d’entreprise sont interdits ; le rattachement initial d’un profil sans entreprise est réservé au serveur. Les uploads de bibliothèque/logos passent par `uploadFileToStorage` qui ajoute l’entreprise au préfixe. Ne pas recréer les anciens chemins globaux ni attribuer automatiquement le catalogue à une nouvelle société. Tests complets : `security-db-tenant.sql`, uniquement sur PostgreSQL jetable, crée sa propre base séparée. Voir le §24 de `docs/FEATURES.md` pour reprise, limites et déploiement.

Le huitième lot ajoute `/compte/saisies`, la comparaison guidée et les copies de résolution dans IndexedDB version 3. Un choix doit comparer les versions locales et serveur observées, conserver les anciens brouillons et photos dans la même transaction, puis passer par la CAS existante. Ne jamais réutiliser une ancienne opération ni acquitter une révision différente. L’import v1/v2 vérifie le propriétaire, conserve les copies archivées, renomme les photos importées et place toute saisie active dans la récupération sans base inventée ; aucune synchronisation automatique d’un import. La base historique sans propriétaire reste isolée. Les copies locales ne remplacent pas les archives serveur de S06. Tests : `offline-resolution.test.cjs` avec les autres suites. Voir le §25 de `docs/FEATURES.md`.

Le neuvième lot (063) exige la préparation serveur de l’archive avant `cloturer_visite`. L’empreinte couvre le contexte et le référentiel complet ; les octets des fichiers sont copiés et hachés. La clôture et l’archive sont publiées dans la même transaction. `visite_archives` est en ajout seul ; les nouveaux PDF lisent exclusivement ses copies et vérifient leurs empreintes. Les reprises historiques sont explicitement datées au moment de copie et n’inventent pas une preuve passée. Ne jamais remplacer les fichiers de sources, réouvrir une visite pour corriger son archive ou réintroduire une génération depuis le référentiel vivant. Les invités n’accèdent pas aux annexes réglementaires par les copies. Voir §26 de `docs/FEATURES.md` et `security-db-archives.sql` (base jetable après les tests d’isolation).
...
Deux entreprises fictives temporaires ont passé 30 contrôles GoTrue/TOTP, PostgREST, Storage et API avec des sessions utilisateur réelles. Le nettoyage des fixtures HTTP et leur absence ont été vérifiés. Résultats et limites : `docs/recette-terrain-2026-09-15.md`, spécification §33. Le chantier « RECETTE MOBILE — 15 septembre 2026 » et sa visite restent disponibles pour la manipulation sur appareil physique ; le relevé serveur montre une visite en cours avec une réponse. Ne pas marquer la recette mobile terminée avant les observations iPhone/iPad et la vérification serveur de la reprise réseau. Ce lot ne modifie ni code applicatif ni base : mise à jour du dépôt VPS après CI, image conservée.

Suivi après déclaration « manipulation faite » / « Safari » : les deux visites de recette sont clôturées. La seconde contient une réponse et une photo JPEG vérifiée par HEAD, son rapport est généré sans email. Aucun texte de remarque n’est enregistré. Le scénario mode avion + Wi-Fi coupé et la conservation après rechargement physique restent à confirmer. Conserver les sources clôturées ; ne pas les rouvrir ou les réécrire pour compléter la recette. Les preuves et limites sont actualisées dans le même procès-verbal.
