...
Maintenance du 22 septembre 2026 : les deux visites de « AA - Chantier Test » (dont une terminée avec rapport envoyé) ont été supprimées sur demande explicite du propriétaire, via `scripts/maintenance/20260922-supprimer-visites-chantier-test.sql` (sauvegarde dans `maintenance_privee`, protections rétablies avant commit, fichiers retirés par l’API Storage). Exception ponctuelle, refusant un rejeu ; ne pas la généraliser aux clients. Spécification §37.
